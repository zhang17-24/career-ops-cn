...
开始新建、修复或更新前，必须完整阅读项目 `docs/招聘源适配验收SOP.md`。该文件规定真实数据、双阶段验收、证据记录和启用后的信任检查；离线 fixture 通过不代表真实接入成功。
...
8. 按 SOP 执行有授权的在线验证：一页列表、最多三个详情，保存 `ACCEPTANCE.md`；未完成须标为待在线验收。用户审核启用后的页面扫描另按 SOP 第二阶段执行，只选该企业。
